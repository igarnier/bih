...
module Elt =
  struct
    ...
    let extents { Scene.vbuffer } { t1; t2; t3 } =
...
let intersect_ray { Scene.vbuffer; tbuffer; bbuffer; nbuffer } tri_index ray tmin tmax tri_start tri_end =
  let min_hit = ref NoHit in
  for i = tri_start to tri_end do
    let { t1; t2; t3 } = tbuffer.(tri_index.(i)) in
    if Float3.dot ray.normal nbuffer.(tri_index.(i)) > 0.0 then
      ()
    else
      let v1  = vbuffer.(t1) and
      v2  = vbuffer.(t2) and
      v3  = vbuffer.(t3) in
      ...
      (match res  with
       | Some ({ t = t } as hit) ->  
         (match !min_hit with
          | NoHit -> min_hit := (Hit(tri_index.(i), hit))
          | Hit(_, { t = t' }) -> 
...
let rec traverse obj tri_index ray tmin tmax bih =
  if tmin >= tmax then NoHit
    ...
    | Bih.Node { axis; leftclip; rightclip; left; right } ->
      (* TODO : dim <- axis *)
        if ray.normal.(axis) >= 0.0 then
        begin
          (* going left-to-right : first left, then right *)
          let ray_start = ray.origin.(axis) +. ray.normal.(axis) *. tmin in
          let ray_end   = ray.origin.(axis) +. ray.normal.(axis) *. tmax in
          (if ray_start <= leftclip then
             (* going through left subtree *)
             (let far_clip = fmin ((leftclip -. ray.origin.(axis)) *. ray.inormal.(axis)) tmax in
              let left_hit = traverse obj tri_index ray tmin far_clip left in
              if leftclip <= rightclip then
                match left_hit with
                | NoHit -> 
                  if rightclip <= ray_end then
                    let near_clip = fmax ((rightclip -. ray.origin.(axis)) *. ray.inormal.(axis)) tmin in
                    traverse obj tri_index ray near_clip tmax right
                  else
                    NoHit
                | Hit _ -> left_hit (* early exit *)
              else if rightclip <= ray_end then
                let near_clip = fmax ((rightclip -. ray.origin.(axis)) *. ray.inormal.(axis)) tmin in
                let right_hit = traverse obj tri_index ray near_clip tmax right in
                (match (left_hit, right_hit) with
                 | NoHit, NoHit -> NoHit
                 | NoHit, x
                 | x, NoHit -> x
                 | Hit(_, { t = t1 }), Hit(_, { t = t2 }) -> 
        ...
        begin
          ...
          if rightclip <= ray_start then
            ...
            (let far_clip  = fmin ((rightclip -. ray.origin.(axis)) *. ray.inormal.(axis)) tmax in
             let right_hit = traverse obj tri_index ray tmin far_clip right in
             if leftclip < rightclip then
               match right_hit with
               | NoHit -> 
                 if ray_end <= leftclip then
                   let near_clip = fmax ((leftclip -. ray.origin.(axis)) *. ray.inormal.(axis)) tmin in
                   traverse obj tri_index ray near_clip tmax left
                 else
                   NoHit
               | Hit _ -> right_hit (* early exit *)
...
let intersect_ray_shadow { Scene.vbuffer; tbuffer; bbuffer; nbuffer } tri_index ray tmin tmax tri_start tri_end =
...
let rec traverse_shadow obj tri_index ray tmin tmax bih =
  if tmin >= tmax then
    ...
    | Bih.Leaf { start; stop } ->
      intersect_ray_shadow obj tri_index ray tmin tmax start stop
      ...
      if ray.normal.(axis) >= 0.0 then
        begin
          ...
          (if ray_start <= leftclip then
             (* going through left subtree *)
             (let far_clip = fmin ((leftclip -. ray.origin.(axis)) *. ray.inormal.(axis)) tmax in
              let left_hit = traverse_shadow obj tri_index ray tmin far_clip left in
        ...
        begin
          ...
          if rightclip <= ray_start then
            (* going through right subtree *)
            (let far_clip  = fmin ((rightclip -. ray.origin.(axis)) *. ray.inormal.(axis)) tmax in
             let right_hit = traverse_shadow obj tri_index ray tmin far_clip right in
