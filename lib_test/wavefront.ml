...
type result = Vertex of float * float * float | Face of int * int * int
...
let vertex =
  ...
  match fl with
  | [x; y; z] -> return (Some (Vertex (x, y, z)))
  | _ -> failwith "vertex: error"

let texture_coord =
  string "vt" >>= fun _ ->
  spaces >>= fun _ -> sep_by spaces float
...
let wavefront_line = choice [vertex; triangle; comment; mtllib; usemtl; group]
...
let load :
 ...
 fun ~filename ~scale ~material ~position ~rotation ~scene ->
  if not (Sys.file_exists filename) then
    Format.kasprintf invalid_arg "%s does not exist, exiting" filename ;
  ...
  (try
     while true do
       ...
       match parse_string ~consume:Consume.Prefix wavefront_line line with
       | Result.Error _ | Result.Ok None -> ()
       | Result.Ok (Some (Vertex (x, y, z))) ->
           let v = V3.v x y z in
           let v = V3.add (V3.ltr rotation (V3.smul scale v)) position in
           vertices := v :: !vertices
  ...
  Printf.printf
    "loading %s:\n%d vertices\n%d triangles\n"
    filename
    (Array.length vbuffer)
    (Array.length tbuffer) ;
  Printf.printf
    "object bbox pos: %f %f %f to %f %f %f\n"
    bbox.mins.(0)
    bbox.mins.(1)
    bbox.mins.(2)
    bbox.maxs.(0)
...
let remove_head s = String.sub s 2 (String.length s - 2)
...
let count_spaces s =
  ...
  for i = 1 to String.length s - 2 do
    ...
    let c2 = s.[i] in
    let c3 = s.[i + 1] in
    if c1 <> ' ' && c2 = ' ' && c3 <> ' ' then incr c else ()
  done ;
  !c

let of_array (x : float array) = V3.v x.(0) x.(1) x.(2)
...
let load_ext :
 ...
 fun ~filename ~scale ~material ~position ~rotation ~scene ->
  ...
  (try
     while true do
       ...
       if String.length line = 0 then ()
         ...
         let v =
           Scanf.sscanf (remove_head line) " %f %f %f " (fun x y z ->
               let v = V3.v x y z in
               V3.add (V3.ltr rotation (V3.smul scale v)) position)
         in
         vertices := v :: !vertices
  ...
  Printf.printf
    ...
    bbox.maxs.(1)
    bbox.maxs.(2) ;
  let diff = V3.sub (of_array bbox.maxs) (of_array bbox.mins) in
  Printf.printf
    "object bbox extents: %f %f %f\n"
    (V3.x diff)
    (V3.y diff)
    (V3.z diff) ;
  scene
